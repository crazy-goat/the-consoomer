...
- With `retry` enabled, the declaration runs inside the retried operation, so a transient declaration failure is retried instead of escaping `send()` (the connection heartbeat is checked before setup, too)
