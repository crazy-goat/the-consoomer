...
class DsnParserOptionsTest extends TestCase
{
    ...
    public function testFanoutExchangeType(): void
    {
        $fanoutExchange = 'test_fanout_exchange';
        $fanoutQueue1 = 'test_fanout_queue_1';
        $fanoutQueue2 = 'test_fanout_queue_2';

        $this->declareExchange($fanoutExchange, 'fanout');
        $this->declareQueue($fanoutQueue1);
        $this->declareQueue($fanoutQueue2);
        $this->bindQueue($fanoutQueue1, $fanoutExchange);
        $this->bindQueue($fanoutQueue2, $fanoutExchange);
        ...
        try {
            ...
            $dsn = sprintf(
                'amqp-consoomer://%s:%s@%s:%d/%s/%s?exchange_type=fanout&queues[queue1]=%s&queues[queue2]=%s',
                $user,
                $password,
                $host,
                $port,
            ...
            $testMessage = new \stdClass();
            $testMessage->content = 'Fanout test';
            $envelope = new Envelope($testMessage);
            ...
            $this->deleteQueue($fanoutQueue1);
            $this->deleteQueue($fanoutQueue2);
            $this->deleteExchange($fanoutExchange);
        }
    }
}
