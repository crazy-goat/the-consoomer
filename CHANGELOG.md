...
### Fixed
- `Receiver::get()` now runs a single consume loop on the shared channel in multi-queue mode, instead of calling `consume()` once per queue. Each idle queue used to block the full `read_timeout`, so an idle `get()` over N queues took N × `read_timeout` and a message published to a later queue waited until every earlier consumer had timed out; deliveries were also attributed to whichever queue's loop happened to be current, not the queue that actually owned the consumer. The owning queue is now resolved from the envelope's consumer tag captured at `connect()` time, so one loop serves every consumer and the batch stops at the global `batch_size` (#309)
