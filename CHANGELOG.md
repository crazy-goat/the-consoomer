...
### Fixed
- The bundled `examples/symfony` app no longer pins the EOL Symfony `7.2.*` line — outside the library's supported range (`^6.4 || ^7.4 || ^8.0`); the example now requires `^7.4` across `console`/`dotenv`/`framework-bundle`/`messenger`/`runtime`/`yaml` (locked at 7.4.x, which includes `CloseableTransportInterface`), with `extra.symfony.require` aligned to `^7.4`, and `composer install` / the publish + consume flows were verified against a live broker (#241)
