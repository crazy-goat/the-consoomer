...
- Delayed-message sends now validate the routing key before it is templated into AMQP entity names. `Sender::sendWithDelay()` rejects a routing key containing anything other than `A-Z a-z 0-9 . _ -` or longer than 255 bytes, and rejects a delay queue name that would exceed the AMQP 255-byte limit. Previously a publisher-controlled routing key (often derived from message content or tenant IDs) was interpolated verbatim into the delay queue name, the `x-dead-letter-routing-key`, and the retry routing key, letting a publisher create arbitrarily named durable queues and dead-letter routes broker-side (#289). Bounding the number of distinct delay queues remains tracked by #211
