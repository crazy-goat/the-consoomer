...
### Fixed
- DSN query string can no longer silently disable TLS on an `amqps-consoomer://` (or legacy `amqps://`) scheme — `?ssl=false` now throws `\InvalidArgumentException` instead of downgrading the connection to cleartext on port 5671 with no warning. The `ssl` flag is pinned by the scheme and is non-overridable from the query string; use `amqp-consoomer://` for a cleartext connection. Additionally, `AmqpFactory::configureSsl()` now logs a `warning` (previously `debug`) whenever peer-certificate verification is disabled (`ssl_verify=false`) (#286)
